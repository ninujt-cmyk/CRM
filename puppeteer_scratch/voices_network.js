...
(async () => {
    ...
    try {
      ...
      if (!url.includes('.js') && !url.includes('.css') && !url.includes('.svg') && !url.includes('.png')) {
        ...
        if (type.includes('json') || type.includes('text/x-component') || url.includes('api')) {
          ...
          responses.push({ url, type, text: text.substring(0, 5000) });
        }
      }
    } catch (e) {}
  });
  ...
  const fs = require('fs');
  fs.writeFileSync('voices_page_network.json', JSON.stringify(responses, null, 2));
  console.log("Wrote voices_page_network.json");
