      ...
      <div className="flex flex-col lg:flex-row gap-3 p-1">
        <div className="flex-1 relative">
          ...
          <Input 
            ...
            className="pl-10 bg-white dark:bg-slate-950 border-slate-200 dark:border-slate-800 text-slate-900 dark:text-slate-100 placeholder-slate-400 dark:placeholder-slate-500"
        ...
        <div className="flex gap-2 w-full lg:w-auto overflow-x-auto pb-1 lg:pb-0">
          ...
          <Button 
            ...
            className={cn("whitespace-nowrap", isPowerMode ? "bg-amber-500 hover:bg-amber-600 text-white border-amber-600" : "text-amber-700 border-amber-200 hover:bg-amber-50 dark:text-amber-400 dark:border-amber-900/50 dark:hover:bg-amber-950/20")}
          >
          ...
          <Select value={status} onValueChange={(val) => { setStatus(val); updateFilters('status', val); }}>
            <SelectTrigger className="w-[160px] bg-white dark:bg-slate-950 border-slate-200 dark:border-slate-800 text-slate-900 dark:text-slate-100">
          ...
          <Select value={priority} onValueChange={(val) => { setPriority(val); updateFilters('priority', val); }}>
            <SelectTrigger className="w-[140px] bg-white dark:bg-slate-950 border-slate-200 dark:border-slate-800 text-slate-900 dark:text-slate-100">
          ...
          <Button onClick={applySearch} className="bg-slate-900 hover:bg-slate-800 dark:bg-slate-100 dark:text-slate-900 dark:hover:bg-slate-200" disabled={isPending}>
            {isPending ? <Loader2 className="h-4 w-4 mr-2 animate-spin"/> : <SlidersHorizontal className="h-4 w-4 mr-2" />} 
            Filter
          </Button>
          {hasActiveFilters && (
            <Button variant="outline" onClick={clearFilters} title="Reset" disabled={isPending} className="border-slate-200 dark:border-slate-800">
