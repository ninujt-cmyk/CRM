...
import { unicornSidebarGroups } from "@/config/sidebar-nav"
            ...
            <div
              className={cn(
                ...
                isActive 
                  ...
                  : "bg-white dark:bg-slate-900 text-slate-500 dark:text-slate-400 hover:text-blue-600 dark:hover:text-blue-400 border border-slate-100 dark:border-slate-800"
              )}
            >
    ...
    <Link href={item.href} className="block mb-2 px-2 perspective-[1000px] group/item">
      <div
        className={cn(
          ...
          "hover:-translate-y-1.5 hover:scale-[1.02] hover:shadow-[0_20px_25px_-5px_rgba(0,0,0,0.1),0_8px_10px_-6px_rgba(0,0,0,0.1)] hover:border-blue-200/50 dark:hover:border-slate-700/50",
          
          // Active Click Effect (Press down)
          "active:scale-[0.98] active:translate-y-0 active:shadow-none",

          isActive 
            ? "bg-gradient-to-r from-blue-50 via-indigo-50 to-white dark:from-blue-950/40 dark:via-indigo-950/40 dark:to-slate-900/40 text-blue-700 dark:text-blue-400 border-blue-100 dark:border-blue-900/50 shadow-sm" 
            : "bg-transparent text-slate-600 dark:text-slate-400 border-transparent hover:bg-white dark:hover:bg-slate-800/40"
        )}
      >
...
export function UnicornSidebar() {
  ...
  return (
    <>
      {/* 1. MOBILE SIDEBAR */}
      <div className="md:hidden p-4 fixed top-[max(env(safe-area-inset-top),0px)] left-[max(env(safe-area-inset-left),0px)] z-50">
        <Sheet>
    ...
    <div className="flex flex-col h-full bg-transparent">
      ...
      <div className={cn("h-24 flex items-center border-b border-slate-100/50 dark:border-slate-800/50 px-6 transition-all duration-500", isCollapsed ? "justify-center" : "justify-start gap-4")}>
        <div className="relative group cursor-default">
            ...
            <div className="h-12 w-12 bg-white dark:bg-slate-900 border border-slate-100/50 dark:border-slate-800 rounded-full flex items-center justify-center shadow-[0_10px_20px_-5px_rgba(79,70,229,0.4)] flex-shrink-0 transition-all duration-500 group-hover:scale-110 group-hover:rotate-6 group-hover:shadow-blue-600/50 overflow-hidden">
              <Image 
        ...
        {!isCollapsed && (
          <div className="flex flex-col overflow-hidden animate-in fade-in slide-in-from-left-4 duration-500">
            ...
            <span className="text-[10px] text-slate-400 dark:text-slate-500 font-bold uppercase tracking-[0.2em]">Unicorn AI</span>
          </div>
        )}
      </div>
      ...
      <ScrollArea className="flex-1 py-6 min-h-0">
        <div className="px-4 space-y-9">
          <TooltipProvider delayDuration={0}>
            {unicornSidebarGroups.map((group, groupIndex) => {
                ...
                <div className="space-y-1.5">
                  {visibleItems.map((item) => (
                    <SidebarItem 
                      ...
                      isActive={(item as any).exact ? pathname === item.href : pathname.startsWith(item.href)} 
                    />
                  ))}
                </div>
              </div>
      ...
      <div className="p-4 border-t border-slate-100/50 dark:border-slate-800/50 mt-auto">
        {!isCollapsed && (
          <div className="group flex items-center gap-3 px-3 mb-4 p-3 rounded-2xl bg-gradient-to-r from-slate-50 to-white dark:from-slate-900/50 dark:to-slate-950/50 border border-slate-100 dark:border-slate-800 shadow-sm hover:shadow-lg hover:-translate-y-1 transition-all duration-300 cursor-default">
