...
export default function TelecallerLoginsPage() {
    ...
    const dailyGoal = 5 
    const todayCount = logins.filter((l: any) => new Date(l.updated_at).toDateString() === new Date().toDateString()).length
    ...
    const fetchLogins = useCallback(async () => {
        ...
        if (data) {
            ...
            const pending = data.filter((l: any) => l.status === 'Login Done' || l.status === 'Pending')
