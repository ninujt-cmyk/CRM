...
export async function initiateC2CCall(leadId: string, customerPhone: string) {
  try {
    ...
    // 🔴 THE FIX: Dynamically import and create the Admin client ONLY when this function runs on the server.
    // This absolutely prevents Next.js from bleeding the environment variable requirement to the client bundle.
    ...
    const payload = {
        ...
        // We no longer need calledId because the webhook uses Smart Lookup!
        calledId: "" 
    };
