...
export async function getLeaderboardData() {
  ...
  const leaderboard = await Promise.all(users.map(async (user: any) => {
    ...
    const achieved = disbursedLeads?.reduce((sum: number, lead: any) => sum + (Number(lead.loan_amount) || 0), 0) || 0;
