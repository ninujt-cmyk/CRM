  ...
  useEffect(() => {
    ...
    const fetchLogs = async () => {
      ...
      if (data) {
        ...
        const connected = data.filter((d: any) => d.disposition === 'ANSWERED').length;
        const dtmfPressed = data.filter((d: any) => d.digits_pressed !== null && d.digits_pressed !== "").length;
        const totalBillSec = data.reduce((acc: number, curr: any) => acc + (curr.bill_seconds || 0), 0);
        const creditsUsed = data.reduce((acc: number, curr: any) => acc + (curr.credits_used || 0), 0);
